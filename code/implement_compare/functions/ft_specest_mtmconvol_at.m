  ...
  for ifreqoi = 1:nfreqoi
    switch taper
      case 'dpss'
        ...
        case 'hanning'
            htap = hanning(timwinsample(ifreqoi))';
            w_s = tap./norm(tap, 'fro');
            w_s_phase = angle(hilbert(htap));
            N = timwinsample(ifreqoi);
            w_at = zeros(1, N);
            
            for n = 1:N
                w_at(n) = w_s(n) .* exp(k * w_s_phase(n));
            end
            c = N / (2 * sum(w_at)); % normalization factor
            tap = c*w_at;
